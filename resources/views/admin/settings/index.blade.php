                ...
                <div class="panel-body">
                    ...
                    <div class="dataTable_wrapper">
                                                ...
                                                <ul class="nav navbar-center navbar-top-links" style="border-radius: 15px;">
                                                    <li class="dropdown">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-user">
                                                            <li>
                                                                <a href="{{route('admin/settings/edit', $setting->id)}}" style="text-decoration: none; color: white; width: 75px;margin:auto" class="btn btn-success">
                                                                    edit
                                                                </a>
                                                            </li>
                                                            <li class="divider"></li>
                                                            <li >
                                                                <button class="dropdown-item btn btn-danger openDeleteFrom" data-toggle="modal" data-target="#myModalDelete" data-id="{{$setting->id}}">
                                                                    delete
                                                                </button>
                                                            </li>
                                                        </ul>
                        ...
                        <div style="margin-top: 20px; font-weight: 600; font-size: 16px;">
                            Showing 1 to <span id="showItems"></span> of <span>{{App\Models\Setting::count()}}</span> entries
                        </div>
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form role="form" action="{{url(route('admin/settings/delete'))}}" method="get">
                                        {{ csrf_field() }}
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            $(document).on('click', '#load_more_button', function() {
                ...
                function load_data(id = "", _token) {
                    $.ajax({
                        url: "{{ route('admin/settings/getMore') }}",
                        method: "POST",
                        data: {
                            id: id,
                            _token: _token,
                        ...
                        success: function(data) {
                            if (data.length > 0) {
                                for (let i = 0; i < data.length; i++) {
                                    ...
                                    edit_route =  "{{ route('admin/settings/edit') }}" + '/' + data[i].id;
                                    records += `
                                        <tr>
                                            <td>${data[i].id}</td>
                                            <td>${data[i].title}</td>
        ...
        $(document).on('keyup', '#data_search', function() {
            ...
            $.ajax({
                url: "{{ route('admin/settings/search') }}",
                method: "POST",
                data: {
                    query: query,
                    _token: _token
